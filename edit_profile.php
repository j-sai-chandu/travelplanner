    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (!preg_match('/^\d{10}$/', $phone)) {
            ...
            if ($phone_check_result->num_rows !== 0) {
                echo "<script>alert('Phone number already there in use. Please use another mobile number');</script>";
                $stmt->close();
            } else {
                $stmt->close();
                ...
                if (!filter_var($email, FILTER_VALIDATE_EMAIL) || !preg_match('/@gmail\.com$/', $email)) {
                    ...
                    if ($email_check_result->num_rows > 0) {
                        echo "<script>alert('Email already exists in the database. Please use a different Gmail address.');</script>";
                        $stmt->close();
                    } else {
                        $stmt->close();
