...
<button onclick="window.location.href='profile1.php?id1=<?php echo $id1; ?>'" class="profile-icon">
                Profile
                    </button>
            </div>
        </div>
    ...
    <div class="destination-list">
        <?php foreach ($places as $place): ?>
            <div class="destination" onclick="goToDateSelection(<?php echo $place['id']; ?>)">
                <img src="photo/<?php echo strtolower($place['place_name']); ?>.png" 
                     alt="<?php echo htmlspecialchars($place['place_name']); ?>"
                     onerror="this.onerror=null;this.src='photo/newthing.png';">
    ...
    <footer id="footer">
        ...
        <div class="about" id="aboutSection">
            ...
            <p>your ultimate travel planning companion! We're dedicated to making your travel dreams come true by simplifying the trip planning process. Whether you're a frequent flyer, a family on vacation, or a solo adventurer, we're here to guide you every step of the way.</p>
        </div>
    </footer>
    ...
    <script>
        ...
        function subscribeUser() {
            const urlParams = new URLSearchParams(window.location.search);
            const id1 = urlParams.get('id1');
            window.location.href = `payButton.html?id1=${id1}`;
        }
        function goBack() {
            window.history.back();
        }
